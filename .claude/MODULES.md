...
| 4 | **Cart** | Cart schema (per user); add/remove/update qty; **persists across sessions for logged-in user**; line totals + order total computed server-side | 1,3 | ✅ shipped | `feature/04-cart` |
| 5 | **Checkout & Orders** | Order schema; checkout flow with **mock / Stripe test-mode** payment; create order on success; **decrement stock atomically**; reject ordering > stock; recompute price server-side (no client-trusted totals); order confirmation page | 4 | ✅ shipped | `feature/05-checkout-orders` |
| 6 | **Order history (customer)** | list-my-orders endpoint (own orders only) + status; customer order history page | 5 | ✅ shipped | `feature/06-account-orders` |
| 7 | **Admin — product management** | admin-only CRUD for products; image upload **or** URL (document choice); admin product table + create/edit modal + delete confirm | 2 (admin guard from 0) | ✅ shipped | `feature/07-admin-products` |

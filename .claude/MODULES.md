...
|---|--------|-----------------|------------|--------|----------------|
| 0 | **Foundation** | DB connection (done), shared response/error filter, `RolesGuard` + `@Roles()`, FE Redux `store/` + API/token layer + route guards + route constants, **seed script** (admin + customer + sample products) | — | ⬜ todo | |

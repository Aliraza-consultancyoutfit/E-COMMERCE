...
| 2 | **Product catalog (storefront read)** | Product schema (name, description, price, image, category, stock); list endpoint with **search + category filter + price range + sort (price/newest) + pagination**; storefront catalog page with filters/sort/pagination using shared table/cards | 0,1 | 🟡 in-progress (built+verified, awaiting ship) | |
